...
export function CompactGlobalAIChatBar({ activePanelLabel }) {
  ...
  const shellBg = isDark ? '#070d20' : '#eef6ff'
  const controlBg = isDark ? '#171d3a' : '#ffffff'
  ...
  return (
    <form
      aria-label={isVi ? 'Thanh chat AI chung' : 'Shared AI chat bar'}
      onSubmit={(event) => {
    ...
    >
      ...
      <button
        ...
        disabled={busy}
        title={isVi ? 'Đính kèm file cho chatbot AI chung' : 'Attach files to shared AI chatbot'}
        style={{ border: `2px solid ${border}`, borderRadius: 26, background: controlBg, color: isDark ? '#dbe5ff' : '#1e3a8a', fontSize: 34, fontWeight: 900, cursor: busy ? 'not-allowed' : 'pointer' }}
      >
        +
      </button>
      <textarea
        ...
        style={{ resize: 'none', minHeight: 92, border: `2px solid ${border}`, borderRadius: 24, padding: '22px 24px', outline: 'none', font: 'inherit', fontSize: 24, lineHeight: 1.45, color: text, background: controlBg }}
      />
      <button
        type="button"
        onClick={toggleMic}
        ...
        style={{ border: `2px solid ${recording ? '#ef4444' : border}`, borderRadius: 26, background: recording ? 'linear-gradient(135deg,#ef4444,#dc2626)' : controlBg, color: recording ? '#fff' : text, fontSize: 30, cursor: transcribing ? 'wait' : 'pointer' }}
      >
      ...
      <button
        ...
        disabled={busy}
        title={isVi ? 'Tải hình ảnh cho AI phân tích' : 'Upload image for AI analysis'}
        style={{ border: `2px solid ${attachedFiles.length ? '#14b8a6' : border}`, borderRadius: 26, background: controlBg, color: text, fontSize: 30, cursor: busy ? 'not-allowed' : 'pointer' }}
      >
        🖼️
      </button>
      <button
        ...
        style={{ border: 'none', borderRadius: 26, color: '#d8e7ea', background: 'linear-gradient(135deg,#187c86,#0f6674)', fontWeight: 900, fontSize: 24, cursor: busy ? 'wait' : 'pointer', opacity: busy || (!input.trim() && attachedFiles.length === 0) ? 0.65 : 1 }}
      >
        {busy ? '...' : (isVi ? 'Gửi' : 'Send')}
      </button>
      {attachedFiles.length > 0 && (
        <div style={{ gridColumn: '1 / -1', color: muted, fontSize: 12, fontWeight: 800 }}>
          {isVi ? `Đã đính kèm ${attachedFiles.length} file` : `${attachedFiles.length} file(s) attached`}
        </div>
      )}
    </form>
  )
