...
function allDots() {
  const map = new Map()
  BODY_AREAS.forEach(area => area.dots.forEach(([x, y], index) => map.set(`${area.id}-${index}`, { x, y, areaId: area.id, areaLabel: area.label })))
...
export default function MyPainPathBodyPanel() {
  ...
  const chips = mode === 'standard'
    ? selectedAreas.map(id => ({ id, label: BODY_AREAS.find(area => area.id === id)?.label || id }))
