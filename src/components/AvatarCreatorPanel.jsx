...
export default function AvatarCreatorPanel() {
  ...
  const loadAnimationFile = async (animation) => {
    ...
    console.log(`Starting animation load: ${animationPath}`)
    console.log('Fetching animation file')
    console.log(`Direct fetch for animation: ${animationPath}`)
  ...
  const selectedModelUrl = selectedAvatar?.model_file_url || selectedAvatar?.vrm_url || ''
        ...
        <div style={{ padding: 18, display: 'grid', gridTemplateColumns: 'minmax(260px, 360px) 1fr', gap: 18 }}>
          <aside style={{ border: `1px solid ${palette.border}`, borderRadius: 20, padding: 16, background: palette.card2, alignSelf: 'start' }}>
            ...
            <div style={{ marginTop: 16, border: `1px solid ${palette.border}`, borderRadius: 18, overflow: 'hidden', background: isDark ? '#070b16' : '#f8fafc' }}>
              ...
              <div style={{ position: 'relative', minHeight: 300, overflow: 'hidden', background: isDark ? 'radial-gradient(circle at 50% 20%, rgba(0,229,255,0.18), transparent 38%), linear-gradient(180deg,#0b1220,#050816)' : 'radial-gradient(circle at 50% 20%, rgba(0,184,204,0.16), transparent 40%), linear-gradient(180deg,#f8fafc,#e5e7eb)' }}>
                <div style={{ position: 'absolute', inset: 'auto 18px 18px', height: 80, borderRadius: '50%', border: `1px solid ${isDark ? 'rgba(148,163,184,0.28)' : 'rgba(71,85,105,0.22)'}`, transform: 'perspective(360px) rotateX(62deg)', background: isDark ? 'repeating-radial-gradient(circle, rgba(148,163,184,0.24) 0 1px, transparent 1px 28px)' : 'repeating-radial-gradient(circle, rgba(71,85,105,0.22) 0 1px, transparent 1px 28px)' }} />
                <div style={{ position: 'absolute', left: '50%', bottom: 42, width: 2, height: 178, transform: 'translateX(-50%)', background: isDark ? 'linear-gradient(transparent,rgba(0,229,255,0.3),transparent)' : 'linear-gradient(transparent,rgba(0,184,204,0.28),transparent)' }} />
                {selectedModelUrl ? (
                  <model-viewer
                    ...
                    style={{ position: 'relative', width: '100%', height: 300, display: 'block', background: 'transparent' }}
                  />
                ) : (
                  <div style={{ position: 'relative', zIndex: 2, height: 300, display: 'grid', placeItems: 'center', perspective: 900 }}>
                    <div style={{ width: 150, height: 210, borderRadius: 26, background: 'linear-gradient(145deg,rgba(255,255,255,0.16),rgba(255,255,255,0.04))', border: `1px solid ${isDark ? 'rgba(255,255,255,0.18)' : 'rgba(15,23,42,0.12)'}`, boxShadow: '0 22px 55px rgba(0,0,0,0.24)', transform: 'rotateY(-12deg) rotateX(4deg)', display: 'grid', placeItems: 'center', overflow: 'hidden' }}>
                      {selectedAvatar?.thumbnail_url
                        ? <img src={selectedAvatar.thumbnail_url} alt={selectedAvatar.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            ...
            <div style={{ marginTop: 16, border: `1px solid ${palette.border}`, borderRadius: 18, overflow: 'hidden', background: isDark ? '#090c14' : '#f5f1ea' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr minmax(116px, 138px)', minHeight: 360 }}>
                ...
                <div style={{ padding: 12, borderLeft: `1px solid ${palette.border}`, background: isDark ? 'rgba(255,255,255,0.035)' : 'rgba(255,255,255,0.52)', maxHeight: 360, overflowY: 'auto' }}>
                  ...
                  {animationLoadStatus && (
                    <div style={{ marginTop: 10, padding: '8px 9px', borderRadius: 8, background: isDark ? 'rgba(0,229,255,0.10)' : 'rgba(0,184,204,0.10)', color: '#00b8cc', fontSize: 11, fontWeight: 800, lineHeight: 1.35 }}>
                      {animationLoadStatus}
                    </div>
                  )}
                </div>
