...
import { makeImageInBodyRecord, parseInBodyCsv, recordsToInBodyCsv, summarizeInBodyRecords } from '../../lib/inbodyCsv.js'

// ─── Constants ────────────────────────────────────────────────────────────────
const TYPE_COLORS = {
  xray: '#00e5ff', ct: '#9c6fff', mri: '#f48fb1', pdf: '#ffb74d', photo: '#00e676', video: '#83f7ff', csv: '#b3ff5f',
}
const ACCEPT  = 'image/jpeg,image/png,image/webp,image/gif,image/heic,image/heif,application/pdf,text/csv,.csv'
const MAX_MB  = 20
...
export default function MedicalUploader({ patientId, onSelectImage }) {
  ...
  async function convertInBodyImageToCsv(record) {
    if (!record || record.fileType === 'csv' || !record.mimeType?.startsWith('image/')) return
    const csvRecord = makeImageInBodyRecord(record)
    const csvText = recordsToInBodyCsv([csvRecord])
    const base64Data = btoa(unescape(encodeURIComponent(csvText)))
    const loginName = (user?.name || user?.email || 'User').toString().replace(/[^a-z0-9._-]+/gi, '_')
    const filename = `InBody_Image_${loginName}_${csvRecord.rawDate}.csv`
    const csvMedicalRecord = {
      ...
      base64Data,
      textContent: csvText,
      notes: `Converted InBody image → CSV 1 dòng · source ${record.filename}`,
    ...
    await saveRecord(csvMedicalRecord, {
    ...
    notifyUpload()
    await loadRecords()
    setSelected(csvMedicalRecord)
    setNotes(csvMedicalRecord.notes)
    setView('detail')
  }
          ...
          <div style={{ display: 'grid', gridTemplateColumns: selected.fileType === 'csv' ? 'minmax(0,1.35fr) minmax(320px,0.65fr)' : '1fr 1fr', gap: 20 }}>
            ...
            <div>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: 14 }}>
                ...
                {selected.mimeType?.startsWith('image/') && selected.fileType !== 'csv' && (
                  <button type="button" onClick={() => convertInBodyImageToCsv(selected)} style={{
                    marginTop: 12, width: '100%', padding: '10px 12px', borderRadius: 10, cursor: 'pointer',
                    border: '1px solid rgba(179,255,95,0.28)', background: 'rgba(179,255,95,0.10)',
                    color: '#b3ff5f', fontWeight: 800, fontSize: 12,
