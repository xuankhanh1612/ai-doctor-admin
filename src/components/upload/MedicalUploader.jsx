...
import { buildManualInBodyRecordFromPrompt, inBodyRecordsToCsv, parseInBodyCsv, summarizeInBodyRecords } from '../../lib/inbodyCsv.js'
...
function isSupportedUploadFile(file) {
  ...
  return file.type.startsWith('image/') || name.endsWith('.heic') || name.endsWith('.heif') || file.type === 'application/pdf' || isCsvFile(file)
}
...
export default function MedicalUploader({ patientId, onSelectImage }) {
  ...
  const convertInBodyImageToCsv = useCallback((record) => {
    if (!record || !record.mimeType?.startsWith('image/')) return
    const defaultDate = new Date(record.uploadedAt || Date.now()).toISOString().replace(/\D/g, '').slice(0, 14)
    const sample = `${defaultDate},74.5,27.7,24.5,27.4,32.8,1451,64,36.7,0.379,10,5.7`
    const input = window.prompt(
      lang === 'vi'
        ? 'Nhập 12 thông số đọc từ hình InBody theo thứ tự: ngày YYYYMMDDHHmmss, cân nặng, cơ xương, mỡ cơ thể kg, BMI, % mỡ, BMR, điểm InBody, nước L, ECW, mỡ nội tạng Level, góc pha. Có thể sửa giá trị mẫu bên dưới.'
        : 'Enter 12 InBody values from the image: date YYYYMMDDHHmmss, weight, skeletal muscle, body fat mass kg, BMI, fat %, BMR, InBody score, water L, ECW, visceral fat level, phase angle. You can edit the sample below.',
      sample,
    )
    if (!input) return
    const csvRecord = buildManualInBodyRecordFromPrompt(input, record)
    const csvText = inBodyRecordsToCsv([csvRecord])
    const baseName = (record.filename || 'inbody_image').replace(/\.[^.]+$/, '')
    const file = new File([csvText], `${baseName}_converted_inbody.csv`, { type: 'text/csv' })
    processFiles([file])
  }, [lang, processFiles])
                ...
                {selected.fileType === 'csv' ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '12px 0' }}>
                    ...
                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center' }}>
                      ...
                      >🔬 {uploadText(lang, 'useForCompare')}</button>
                      <button
                        onClick={() => convertInBodyImageToCsv(selected)}
                        style={{
                          padding: '10px 18px', background: 'rgba(179,255,95,0.12)',
                          border: '1px solid rgba(179,255,95,0.35)', borderRadius: 10, color: '#b3ff5f', fontSize: 13, fontWeight: 800, cursor: 'pointer',
                        }}
