...
import { buildInBodyCsv, extractInBodyMetricsFromText, parseInBodyCsv, summarizeInBodyRecords } from '../../lib/inbodyCsv.js'
...
function textToDataUrl(text, mimeType = 'text/csv') {
  return `data:${mimeType};base64,${textToBase64(text)}`
}

function makeConvertedImageRecord(record) {
  ...
  return {
    ...
    conversionStatus: hasMetrics ? 'metrics-detected-from-text' : 'needs-review',
  }
}

function isSupportedUploadFile(file) {
  return isImageLikeFile(file) || file.type === 'application/pdf' || isCsvFile(file)
}
...
export default function MedicalUploader({ patientId, onSelectImage }) {
  ...
  async function convertSelectedInBodyImageToCsv() {
    if (!selected) return
    const converted = makeConvertedImageRecord({ ...selected, notes })
    const csvText = buildInBodyCsv([converted])
    const filename = `${selected.filename?.replace(/\.[^.]+$/, '') || 'InBody_Image'}_converted.csv`
    const csvRecord = {
      id: `inbody_image_csv_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`,
      filename,
      name: filename,
      fileType: 'csv',
      type: 'csv',
      ...
      textContent: csvText,
      notes: converted.conversionStatus === 'needs-review'
        ? `Converted from InBody image ${selected.filename}. Chưa có OCR local nên các chỉ số trống cần bổ sung trong ghi chú/tên file để parse tự động.`
        : `Converted from InBody image ${selected.filename}.`,
      ownerEmail: user?.email || null,
      ownerName: user?.name || '',
      ownerAvatar: user?.avatar || '',
      ownerProvider: user?.provider || '',
    ...
    await saveRecord(csvRecord, { ownerEmail: user?.email })
    notifyUpload()
    await loadRecords()
    setSelected(csvRecord)
    setNotes(csvRecord.notes || '')
    setView('detail')
  }
          ...
          <div style={{ display: 'grid', gridTemplateColumns: selected.fileType === 'csv' ? 'minmax(0,1.35fr) minmax(320px,0.65fr)' : '1fr 1fr', gap: 20 }}>
            ...
            <div>
              ...
              <div style={{ marginTop: 12 }}>
                ...
                {selected.fileType !== 'csv' && selected.mimeType !== 'application/pdf' && !selected.mimeType?.startsWith('video/') && (
                  <button onClick={convertSelectedInBodyImageToCsv} style={{
                    marginTop: 8, marginLeft: 8, padding: '8px 18px', borderRadius: 8, cursor: 'pointer',
                    background: 'rgba(179,255,95,0.1)', border: '1px solid rgba(179,255,95,0.28)',
                    color: '#b3ff5f', fontSize: 12, fontWeight: 800,
                  }}>📈 Convert InBody Image thành .CSV</button>
                )}
              </div>
